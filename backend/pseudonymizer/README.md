...
- **形状**（時間帯別の接続数カーブ、曜日パターン、チャネル設計、AP モデル、ベンダー分布）
- **相対時刻**（タイムシフトは全ファイル一律のオフセット加算なので、時刻差はそのまま残る）

タイムシフト量は**週単位**で選ぶため、曜日と時刻は保存され、日付だけが失われる。
これは分析結果を壊さないための意図的な設計であり、同時に「曜日・時間帯は手がかりとして残る」
ことを意味する。
...
ファイル名パターンで種別を判定する。判定できない場合はエラー停止。

| 種別キー | ファイル名パターン | 列数 |
|---|---|---|
| `ap_metrics` | `ap_metrics_*.csv` | 33 |
| `ap_events` | `ap_events_*.csv`（`ap_events_backfill_*.csv` を含む） | 11 |
| `client_metrics` | `client_metrics_*.csv` | 36 |
| `sle_metrics` | `sle_metrics_*.csv` | 28 |
| `floormap_summary` | `floormap_*_summary.csv` | 8 |
...
（`ap_metrics` の `mac` は AP の MAC、`client_metrics` の `mac` は端末の MAC）。

新しいファイル種別を追加するときは `schemas.py` に `FileType` を 1 つ足すだけでよい
（例: 今後追加予定の `rf_neighbors`）。
...
| `AP_MAC` | `mac`(ap_metrics), `ap_mac`, `bssid` | `020` + 連番 9 桁 hex |
...
| 規則名 | 内容 |
|---|---|
| `uuid_not_pseudonymized` | UUID 形式（生成した仮名 UUID のパターンは除外） |
