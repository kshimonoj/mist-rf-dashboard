"""分析結果の保存とローテート（``data/rrm_results/``）。

ジョブの一時ディレクトリは破棄されるため、そのままでは調査の記録が残らない。
このモジュールは ``done`` で完了した分析の出力を専用ディレクトリへ「組」として
複製し、溜まりすぎないようローテートする。

``hangap/archive.py`` / ``floorpeak/archive.py`` と同じ設計だが **コードは
共有しない**（import もしない）。ローテートは hangap とも floorpeak とも
``data/logs`` とも独立させる。片方の都合でもう片方の記録が消える構造を作らないため。

**ローテートの設計（過去に実データを全滅させた欠陥の再発防止）**

- 見るのも消すのも ``data/rrm_results/`` の**中だけ**。
...
NAME_PATTERN = re.compile(r"^rrm_result_\d{8}_\d{6}$")

NAME_PREFIX = "rrm_result_"
STAMP_FORMAT = "%Y%m%d_%H%M%S"

DEFAULT_MAX_FILES = 50
DEFAULT_MAX_TOTAL_MB = 500

ENV_MAX_FILES = "RRM_RESULTS_MAX_FILES"
ENV_MAX_TOTAL_MB = "RRM_RESULTS_MAX_TOTAL_MB"
...
def _env_number(key: str, default: float, *, integer: bool) -> float:
    ...
    try:
        ...
        logger.warning(f"rrm archive: {key}={raw!r} を数値として読めません。既定値 {default} を使います")
        return default
    if value <= 0:
        logger.warning(f"rrm archive: {key}={raw!r} は 0 より大きい値が必要です。既定値 {default} を使います")
        return default
    return value
...
def max_total_bytes() -> int:
    """``data/rrm_results/`` の合計サイズ上限（環境変数で上書き可能）。"""
...
def read_meta(result_set: ResultSet) -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning(f"rrm archive: メタ情報を読めません: {path.name}: {e}")
        return {}
...
def delete_set(result_set: ResultSet) -> int:
    ...
    for path in result_set.members.values():
        ...
        try:
            ...
            logger.warning(f"rrm archive: 削除できません: {path.name}: {e}")
            continue
        freed += size
    return freed
...
def rotate(
    ...
    *,
    ...
    if removed:
        logger.info(
            f"[RRM-ROTATE] Deleted {removed} result set(s), freed {freed}B "
