"""フロア別ピーク時点分析のオンデマンドジョブ API。

分析そのものは :mod:`floorpeak.analysis` に集約されており（CLI と同じ関数）、この
...
- ``done`` で完了した結果は ``data/floorpeak_results/`` に組（xlsx/csv/json）として
  保存し、:mod:`floorpeak.archive` がローテートする。保存先は ``data/logs`` の外で、
  :data:`hangap.loader.EXCLUDED_DIR_NAMES` でも除外している。
- **対象サイトは単一指定が必須。**「サイト全体のピーク」は複数サイトでは定義できない。
"""
...
from floorpeak import analysis, archive, loader
from floorpeak.analysis import RESULT_COLUMNS
from hangap import sites as log_sites
from utils import fmt_dt

router = APIRouter(prefix="/api/floorpeak", tags=["floorpeak"])
...
_BODY_FIELDS: frozenset[str] = frozenset({"site", "from", "to", "at"})
...
def _cleanup_tmpdir(td: tempfile.TemporaryDirectory | None, job_id: str) -> None:
    ...
    try:
        ...
        logger.warning(f"floorpeak: temp dir cleanup failed for {job_id}: {e}")
...
def _fail_timed_out() -> None:
    ...
    with _LOCK:
        for job in _JOBS.values():
            ...
            logger.warning(f"floorpeak: job {job.job_id} timed out after {elapsed:.0f}s")
...
def _analyze_and_write(job: _Job, files: list[Path]) -> _Outcome:
    ...
    読み込んだ入力（``ap_metrics`` の DataFrame）は :class:`analysis.AnalysisResult`
    ごとこの関数のスコープに閉じ込め、戻り値には結果（数百行）と meta だけを載せる。
    """
    res = analysis.run_analysis(files, job.params, on_phase=lambda p: _set_phase(job, p))
    _set_phase(job, analysis.PHASE_WRITING)
    tmpdir = tempfile.TemporaryDirectory(prefix="floorpeak_job_")
    base = Path(tmpdir.name)
    stamp = job.started_at.strftime("%Y%m%d_%H%M%S")
    outputs = {
        # xlsx のグラフは既定フロア（端末数が最も多いフロア）。別フロアの xlsx は
        # ダウンロード時に ?floor= でその場で組み立てる
        "xlsx": analysis.write_xlsx(base / f"floorpeak_result_{stamp}.xlsx", res.rows, res.meta),
        "csv": analysis.write_csv(base / f"floorpeak_result_{stamp}.csv", res.rows),
        "summary": analysis.write_summary(base / f"floorpeak_result_{stamp}_summary.txt", res.meta),
    }
    return _Outcome(
...
def _archive_outcome(job: _Job, outcome: _Outcome) -> None:
    """``done`` で完了した結果を ``data/floorpeak_results/`` に保存し、ローテートする。
    ...
    try:
        ...
        logger.warning(f"floorpeak: 分析結果を保存できませんでした（{job.job_id}）: {e}")
...
def _run_job(job: _Job) -> None:
    ...
    try:
        ...
        # 対象 0 行もここに来る。「ピークが無かった」ではなく「そもそも分析対象が
        # 無かった」であり、done ではなく failed にする。
        with _LOCK:
            if not job.timed_out:
                job.status = STATUS_FAILED
                job.error = str(e)
    except Exception as e:  # noqa: BLE001 - ワーカースレッドで例外を落とさない
        logger.exception(f"floorpeak: job {job.job_id} failed")
        with _LOCK:
            if not job.timed_out:
                job.status = STATUS_FAILED
...
def _site(body: dict[str, Any]) -> str:
    """対象サイト。**単一指定が必須**（「すべてのサイト」は受け付けない）。

    複数サイトを混ぜると「サイト全体のピーク」が定義できない（別々の建物の
    合計が最大になる時点には意味がない）。
    """
    raw = body.get("site")
    if raw is None:
        raise _bad_request("site", "対象サイトを 1 つ指定してください（省略できません）")
    if not isinstance(raw, str) or not raw.strip():
        raise _bad_request("site", f"site_id は空でない文字列で指定してください: {raw!r}")
    return raw.strip()
...
def _result_response(
    ...
    *,
    ...
    表示側に分岐が増える。行は全 AP（250 行程度）を一度に返す
    （フロア選択とトップ 20 の切り出しは表示側の責務）。
    """
    return {
        "job_id": job_id,
        "name": name,
...
def _download_response(
    ...
    *,
    ...
    ``floor`` を指定しなければ **保存済みのファイルそのもの** を返す（CLI が書いた
    ファイルと同一）。フロアを指定したときだけ、その場で xlsx を組み立てて返す
    （xlsx のグラフは 1 フロア分しか描けないため）。
    """
    if fmt == "xlsx" and floor is not None:
        known = {f["map_name"] for f in meta.get("floors", [])}
        if floor not in known:
            raise _bad_request("floor", f"結果に無いフロアです: {floor!r}")
        return Response(
            content=analysis.xlsx_bytes(rows, meta, floor),
            media_type=_MEDIA_TYPES["xlsx"],
            headers={"Content-Disposition": f'attachment; filename="{filename}"'},
        )
    if stored is None or not stored.is_file():
        raise HTTPException(status_code=404, detail=f"出力ファイルが見つかりません: {fmt}")
    return FileResponse(stored, media_type=_MEDIA_TYPES[fmt], filename=stored.name)
...
def list_log_sites(
    ...
    走査は Hang AP と同じ :mod:`hangap.sites` を使う（同じログから同じ一覧が
    出るべきで、2 つ実装を持つ理由がない）。**「すべてのサイト」は選択肢に含めない。**
    """
    _sweep()
    files = loader.collect_files(LOGS_DIR)
...
def start_analysis(body: dict | None = Body(default=None)):
    ...
    threading.Thread(
        target=_run_job, args=(job,), daemon=True, name=f"floorpeak-{job.job_id[:8]}"
    ).start()
...
def get_job_result(job_id: str):
    ...
    return _result_response(
        job.rows, job.meta, job.warnings, job_id=job.job_id, name=None,
    )
...
def download_job_result(
    ...
    floor: str | None = Query(None, description="xlsx のグラフに出すフロア（既定は端末数最多）"),
):
    """CLI と同じ書式のファイルを返す（書き出しは floorpeak.analysis で共用）。"""
    _sweep()
    job = _get_job(job_id)
    ...
    return _download_response(
        job.rows, job.meta,
        stored=job.outputs.get(format), fmt=format, floor=floor,
        filename=f"floorpeak_result_{stamp}.xlsx",
    )
...
def delete_job(job_id: str):
    ...
    with _LOCK:
        ...
        if job.status == STATUS_RUNNING:
            ...
            logger.info(f"floorpeak: job {job_id} discarded while running")
        else:
            _JOBS.pop(job_id, None)
...
# ---------------------------------------------------------------------------
# 保存済みの分析結果（data/floorpeak_results/）
# ---------------------------------------------------------------------------
...
def _result_name(name: str) -> str:
    ...
    if not archive.is_valid_name(name):
        raise _bad_request(
            "name", f"floorpeak_result_YYYYMMDD_HHMMSS の形式で指定してください: {name!r}"
        )
    return name
...
def _load_saved(name: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(f"floorpeak: 保存済みの csv を読めません: {path.name}: {e}")
        raise HTTPException(
            status_code=409,
            detail=(
...
def get_saved_result_rows(name: str):
    ...
    保存済みの csv / json を読んで返すだけで、**再分析はしない**（保存した時点の
    結果をそのまま見せる）。
    """
    name = _result_name(name)
    rows, meta = _load_saved(name)
    return _result_response(
        rows, meta, list(meta.get("warnings") or []), job_id=None, name=name,
    )
...
def download_saved_result(
    ...
    """保存済みの xlsx / csv を返す（``floor`` 指定時のみその場で組み立てる）。"""
    name = _result_name(name)
    if format not in _MEDIA_TYPES:
        raise _bad_request("format", f"xlsx / csv で指定してください: {format!r}")
    stored = archive.member_path(RESULTS_DIR, name, f".{format}")
    if format == "csv" or floor is None:
    ...
    rows, meta = _load_saved(name)
    return _download_response(
        rows, meta, stored=stored, fmt=format, floor=floor, filename=f"{name}.xlsx",
    )
...
def delete_saved_result(name: str):
    ...
    for result_set in archive.list_sets(RESULTS_DIR):
        if result_set.name == name:
            ...
            logger.info(f"floorpeak: 保存済みの結果を削除しました: {name} ({freed}B)")
