...
import { fetchClientMetrics, ClientInfo, ClientMetric } from "@/lib/api";
...
export default function ClientDetailModal({ client, siteId, onClose }: {
  client: ClientInfo;
  siteId?: string;
  onClose: () => void;
}) {
  ...
  const { data: metrics, isLoading } = useSWR<ClientMetric[]>(
    `client-metrics-${client.mac}-${hours}`,
    () => fetchClientMetrics(client.mac, hours, siteId),
  );
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-4xl rounded-xl shadow-2xl border flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b flex-shrink-0"
          ...
          <h2 className="font-display font-semibold text-base" style={{ color: "var(--cyan)" }}>
        ...
        <div className="p-5 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <InfoItem label="MAC" value={client.mac} />
            <InfoItem label="Manufacturer" value={client.manufacture} />
            <InfoItem label="OS" value={client.os || client.family} />
            <InfoItem label="AP Name" value={client.ap_name} />
            <InfoItem label="SSID" value={client.ssid} />
            <InfoItem label="VLAN" value={client.vlan_id != null ? String(client.vlan_id) : "-"} />
            <InfoItem label="Band / Channel" value={`${bandLabel(client.band)} / ${client.channel ?? "-"}`} />
          </div>

          {/* 時間範囲セレクター */}
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-display font-semibold tracking-wider" style={{ color: "var(--cyan)" }}>
              METRICS HISTORY
            </h3>
