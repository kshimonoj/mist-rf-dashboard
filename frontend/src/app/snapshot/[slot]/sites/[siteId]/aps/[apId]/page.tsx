...
import { fetchSnapshotMetrics, fetchSnapshotRadioConfig, ApMetric, ApRadioConfig } from "@/lib/api";
...
export default function SnapshotApDetailPage({
  params,
}: {
  params: { slot: string; siteId: string; apId: string };
}) {
  ...
  const { data: metrics } = useSWR<ApMetric[]>(
    `snapshot-${slot}-ap-metrics-${apId}-${hours}`,
    () => fetchSnapshotMetrics(slot, apId, hours),
  );

  const { data: radioConfig } = useSWR<ApRadioConfig>(
    `snapshot-${slot}-ap-radio-${apId}`,
    () => fetchSnapshotRadioConfig(slot, apId),
  );
    ...
    <main className="min-h-screen p-6 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <Link href={`/snapshot/${slot}/sites/${siteId}`}
      ...
      <section className={sectionClass} style={sectionStyle}>
        ...
        {latestMetric ? (
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>メトリクスデータなし</p>
        )}
      </section>
      ...
      <section className={sectionClass} style={sectionStyle}>
        ...
        {chartData.length === 0 ? (
          ...
          <div className="space-y-6">
            ...
            {changes.length > 0 && (
              <div className="flex flex-wrap gap-3 pt-2 border-t" style={{ borderColor: "var(--chart-grid)" }}>
              ...
              <tbody>
                {changes.map((c) => (
                  <tr key={c.id} className="border-b transition-colors" style={{ borderColor: "var(--chart-grid)" }}>
                    ...
                    <td className="py-3 px-3" style={{ color: "var(--text-primary)" }}>{c.band}</td>
